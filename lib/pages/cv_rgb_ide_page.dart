...
class CVRGBIDEPage extends StatefulWidget {
  @override
  _CVRGBIDEPageState createState() => _CVRGBIDEPageState();
}

class _CVRGBIDEPageState extends State<CVRGBIDEPage> {
  ...
  String output = "", error = "";
  Uint8List inputImg, outputImgR, outputImgG, outputImgB;
  ...
  Future<void> runPythonScript(String code) async {
    if (!code.trim().startsWith('r,g,b')) {
      ToastUtils.showToastMessage(text: ConstUtils.cv_rgb_error_line1);
      return;
    }
    ...
    final result = await _channel.invokeMethod("runPythonRGBScript", data);
    ...
    setState(
      () {
        ...
        error = result['error'] ??= "";
        outputImgR = (result['graphOutputR']);
        outputImgG = (result['graphOutputG']);
        outputImgB = (result['graphOutputB']);
        data.clear();
      },
    );
  }
                ...
                child: Container(
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      CodeSnippetWidget(
                        code: ConstUtils.cv_rgb_post_code,
                      ),
                      Padding(
                        padding: const EdgeInsets.all(8.0),
                        child: Text(
                        ...
                        child: Row(
                          ...
                          children: [
                            (outputImgR == null)
                                ? Center(
                                    child: Text(
                                      ConstUtils.rImage_note,
                                      style: TextStyle(
                                        color: Colors.black,
                                        fontWeight: FontWeight.w500,
                                        fontSize: 18 *
                                            MediaQueryUtils(context)
                                                .scaleFactor,
                                      ),
                                    ),
                                  )
  ...
  void appendAndRunPythonCode() async {
    ...
    String final_code =
        '$code\nrImage = Image.fromarray(r)\nrImg = io.BytesIO()\nrImage.save(rImg, format="png")\n'
        '\ngImage = Image.fromarray(g)\ngImg = io.BytesIO()\ngImage.save(gImg, format="png")\n'
        '\nbImage = Image.fromarray(b)\nbImg = io.BytesIO()\nbImage.save(bImg, format = "png")\n';
